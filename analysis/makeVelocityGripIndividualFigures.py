...
# Get a visual
def makeFig(sub, cond, t, vData, gData):
    fig, axs = plt.subplots(2, 1, sharex=True)

    axs[0].set_title(sub + ' ' + cond + ' ' + str(t))
    axs[0].plot(vData['time'], vData['vel_diff'], color=colors[4])
    #axs[0].plot(vData['time'], vData['vel_filled'], color=colors[0])
    axs[0].plot(vData['time'], vData['vel_bw'], color=colors[2])

    #axs[0].plot(vData['time'], vData[''], color=colors[4])
    ...
    #axs[0].set_ylim(np.abs(data['ter_velocity'].min()) - 10, np.abs)
    ...
    for cond in conditions:
        for t in trials:
            ...
            if len(vData) == 0:
                ...
                makeFig(sub, cond, t, vData, gData)
